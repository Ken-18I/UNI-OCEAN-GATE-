...
class Game:
    def __init__(self):
        ...
        #Llamamos a Facemesh de la libreria Mediapipe
        self.mp_face_mesh = mp.solutions.face_mesh
        self.mp_drawing = mp.solutions.drawing_utils
        self.mp_drawing_styles = mp.solutions.drawing_styles

        pygame.init()
        pygame.display.set_caption("PRACTICA CALIFICADA 5")

        #Definimos nuestra fuente
    ...
    def update(self, delta_time):
        ...
        if self.lost or not self.started:
            for event in events:
                #definimos ENTER para comenzar el juego
                #cuando no hayamos empezado o cuandoperdimos
                if event.type == KEYDOWN and event.key == K_RETURN:
                    self.initialize()
                    self.started = True
        else:
            #Aumentamos la velocidad segun el tiempo que vaya pasando
            globals.game_speed = 1 + ( (pygame.time.get_ticks() - self.start_time) / 1000) * .1
            self.score = self.score + (delta_time * globals.game_speed)

            for event in events:
                if event.type == ADD_ENEMY:
                    #Agregamos 1 o 2 enemigos para aumentar la dificultad
                    num = random.randint(1,2)
                    for e in range(num):
                        enemy = Enemy()
                        self.enemies.add(enemy)

                    #Actualizamos el timer que define cuando aparecera un nuevo enemigo
    ...
    def process_collisions(self):
        ...
        #En este caso los hacemos con mascaras
    ...
    def render(self):
        ...
        text_score = self.font.render('Puntaje: ' + str(display_score), True, (255,255,255))
        ...
        if self.lost:
            game_over_text = self.font.render('Haz IMPLOSIONADO', True, (255,255,255), (0,0,0))
            game_over_text_rect = game_over_text.get_rect()
            game_over_text_rect.center = (SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2)
            self.screen.blit(game_over_text, game_over_text_rect)

            retry_text = self.smaller_font.render('Enter para volver a jugar', True, (200,200,200), (0,0,0))
        ...
        if not self.started:
            game_over_text = self.font.render('Enter para comenzar', True, (255,255,255), (0,0,0))
            game_over_text_rect = game_over_text.get_rect()
            game_over_text_rect.center = (SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2)
            self.screen.blit(game_over_text, game_over_text_rect)
        ...
        if image is not None:
            ...
            if results.multi_face_landmarks is not None:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    #Definimos las coordenadas de la cara (arriba y abajo)
                    ...
                    #Obtenemos las coordenadas del 'cuadrado' de la cara para poder mostrarlo en la pantalla despues
                    ...
                    #Dejamos algo de espacio alrededor
                    ...
                    #Dibujamos una linea entre los dos puntos seleccionados para tener una idea gráfica
                    ...
                    #Dibujamos un punto rojo en cada extremo seleccionado
    ...
    #Definimos la funcion que detectara la inclinacion de nuestra cabeza tomando como referencia los puntos antes definidos
    def detect_head_movement(self, top, bottom):
        #Obtenemos en angulo en radianes
        radians = math.atan2(bottom[1] - top[1], bottom[0] - top[0])
        #Lo convertimos a grados
        degrees = math.degrees(radians)

        #Definimos el angulo de deteccion entre 70 y 110 (-1 a 1)
        min_degrees = 70
        max_degrees = 110
        ...
        #Definimos al angulo como indicadores de movimientos del submarino
        self.movement = ( ((degrees-min_degrees) / degree_range) * 2) - 1

    def render_camera(self):        
        #Limpiamos coordenadas del cuadro de la cara
